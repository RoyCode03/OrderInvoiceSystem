...
namespace OrderInvoiceSystem.Controllers
{
    ...
    public class OrderController : Controller
    {
        ...
        private async Task SendInvoiceEmail(string toEmail, string customerName, string subject, string htmlBody, byte[] pdfAttachment, bool isBodyHtml = true)
        {
            ...
            if (!MailAddress.TryCreate(toEmail, out var toMailAddress))
            {
                // Log the error and handle it appropriately.  Don't just swallow the exception.
                _logger.LogError($"Invalid 'toEmail' address: {toEmail}"); // Assuming you have an ILogger instance
                return; // Or throw an exception if you want to stop execution
            }

            if (!MailAddress.TryCreate(senderEmail, out var fromMailAddress))
            {
                _logger.LogError($"Invalid 'senderEmail' address: {senderEmail}");
                return;
            }
            ...
            using (var message = new MailMessage())
            {
                message.From = fromMailAddress; // Use the validated MailAddress object
                message.To.Add(toMailAddress);  // Use the validated MailAddress object
                message.Subject = subject;
                message.Body = htmlBody;
                message.IsBodyHtml = isBodyHtml;
